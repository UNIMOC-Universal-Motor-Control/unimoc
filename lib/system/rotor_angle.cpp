...
RotorAngle::update_sin_cos(const float currentAngle) noexcept
{
	...
	const uint16_t idxS = static_cast<uint16_t>(currentAngle / delta) % TABLE_SIZE;
	const uint16_t idxC = (idxS + (TABLE_SIZE / 4)) % TABLE_SIZE;
	const float fract = std::fmod(currentAngle, delta);
	...
	if (currentAngle < 0.0f) [[likely]] { this->sin = -this->sin; }
}


// constructor with angle
constexpr RotorAngle::RotorAngle(const float rotorAngle) noexcept : angle(rotorAngle), sin(0.0f), cos(1.0f)
{
	update_sin_cos(rotorAngle);
}

// sine cosine difference
constexpr RotorAngle
RotorAngle::operator-(const RotorAngle &other) const noexcept
{
	// Using trigonometric identities for subtraction:
	// sin(a - b) = sin(a)cos(b) - cos(a)sin(b)
	// cos(a - b) = cos(a)cos(b) + sin(a)sin(b)
	return RotorAngle(angle - other.angle, sin * other.cos - cos * other.sin,
...
RotorAngle::operator+(const RotorAngle &other) const noexcept
{
	// Using trigonometric identities for addition:
	// sin(a + b) = sin(a)cos(b) + cos(a)sin(b)
	// cos(a + b) = cos(a)cos(b) - sin(a)sin(b)
	return RotorAngle(angle + other.angle, sin * other.cos + cos * other.sin,
					  cos * other.cos - sin * other.sin);
