...
| **Fresh chat** | Start a new VS Code chat with no dependency on the current context. |
| **`/handoff`** | Write a portable markdown file and seed a session anywhere with it. |
| **Subagent** | Send the task to its own context window and get a report back.     |
| **`/handoff`** | Write a summary that seeds a fresh VS Code chat.                 |
...
**2. Is the context irrelevant to what comes next?** Is everything in this session — the exploration, the decisions, the dead ends — disposable? If so, start a **fresh chat**. It is the cheapest move on the board when no context needs to travel.
...
- swapping to a **different agent harness**,
...
**5. Otherwise, use `/handoff`.** Relevant context, same harness, same directory, and you need to stay in the loop — write a focused summary and start a fresh VS Code chat. Include the next phase so the summary keeps what it needs.

`/handoff` is the **default, not the first reach**. It sits at the bottom because the four questions above it are all cheaper or more precise. The failure mode when people start here is a fresh session that is confidently wrong about a decision the summary flattened.
...
| Secondary (`/handoff`) | Lossy       | Less  | Lots         |
