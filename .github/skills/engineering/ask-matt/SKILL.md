...
3. **Branch — is this a multi-session build?**
  - **Yes** → **`/to-spec`** (turn the thread into a spec), then **`/to-tickets`** to split it into tracer-bullet tickets, each declaring its **blocking edges**. On a local tracker that's one file per ticket under `.scratch/<feature>/issues/`, worked blockers-first by hand; on a real tracker the edges become native blocking links, so any ticket whose blockers are done can be grabbed — kick off **`/implement`** per ticket, starting a fresh VS Code chat between tickets when the previous context is no longer useful. Each ticket is self-contained, so the previous context is disposable.
...
Keep steps 1–3 in **one unbroken VS Code chat** so the grilling, spec, and
tickets all build on the same thinking. Each `/implement` can start fresh from
the ticket when the previous context is no longer useful.

The limit on this is the **[smart zone](https://www.aihero.dev/ai-coding-dictionary/smart-zone)**: the window (~150k tokens on state-of-the-art models) within which the model still reasons sharply. If a session approaches it before `/to-tickets`, use `/handoff` and start a fresh VS Code chat at the nearest phase boundary (see Phase boundaries).
...
- **Fresh chat** — start a new VS Code chat when nothing here matters to what's next.
...
- **`/handoff`** — write a summary that seeds a fresh VS Code chat when the context still matters. Use it at the bottom of the tree rather than the first reach.
...
- **`/research`** — delegate reading legwork to a **subagent** when available: it investigates a question against **primary sources**, then leaves a cited Markdown file in the repo. Keep working while it reads. The file it produces is something to take *into* the main flow at `/grill-with-docs` — research feeds the thinking, it doesn't replace it.
